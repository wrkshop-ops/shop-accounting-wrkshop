...
(function () {
  ...
  async function appData() {
    ...
    const [tx, suppliers, products] = await Promise.all([
      ...
      client.from('suppliers').select('*').eq('user_id', user.id).order('supplier_name'),
      client.from('products').select('*').eq('user_id', user.id).order('product_name')
    ]);
    for (const result of [tx, suppliers, products]) if (result.error) throw result.error;
    const records = tx.data.map(txToLegacy);
    const supplierRows = suppliers.data.map(r => ({ SupplierID: r.supplier_id || r.id, SupplierName: r.supplier_name }));
    const productRows = products.data.map(r => ({ ProductID: r.product_id || r.id, ProductName: r.product_name, Category: r.category || '' }));
  ...
  async function call(name, args) {
    ...
    if (name === 'saveTransaction' || name === 'updateTransaction') {
      ...
      const result = name === 'updateTransaction'
        ? await client.from('transactions').update(row).eq('id', JSON.parse(args[0]).transaction_id).eq('user_id', user.id)
    ...
    if (name === 'deleteTransaction') {
      ...
      const result = await client.from('transactions').delete().eq('id', payload.transaction_id || payload.id).eq('user_id', user.id);
      if (result.error) throw result.error;
      return appData();
    }
    ...
    if (name === 'deleteSupplier') {
      const result = await client.from('suppliers').delete().eq('supplier_id', args[0]).eq('user_id', user.id);
      if (result.error) throw result.error;
      return appData();
    }
    if (name === 'saveProduct') {
    ...
    if (name === 'deleteProduct') {
      const result = await client.from('products').delete().eq('product_id', args[0]).eq('user_id', user.id);
      if (result.error) throw result.error;
      return appData();
    }
